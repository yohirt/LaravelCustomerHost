...
class Paymentservice extends Model
{
    ...
    private $monthDay;
    private $yearNow;
    private $expiredPaymentYear = array();
    private $DateCheck;
    ...
    public function getExpiredPaymentCS($idCS)
    {
        ...
        for ($i= $this->yearStart; $i <= $this->yearNow ; $i++) { 
            ...
            $testPaymentYear = 'App\Paymentservice'::where('client_service_id', $idCS)->where('date_period', $this->DateCheck)->get();
            // $testPayment = 'App\Paymentservice'::where('client_service_id', $idCS)->get();

            
            if($testPayment->count() <= 0 ){
                $this->expiredPaymentYear[] = 'App\Paymentservice'::where('client_service_id', $idCS)->where('date_period',$this->DateCheck)->get();
