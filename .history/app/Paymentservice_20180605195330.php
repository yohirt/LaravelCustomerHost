...
class Paymentservice extends Model
{
     ...
     public function getExpiredPaymentForClient($clientId)
    {
        ...
        //Get all middle CS for client_id =>return id(client_service)
        $idCS = Clientservice::where('client_id', $clientId)->pluck('id', 'service_id', 'client_id');
        ...
        foreach ($idCS as $id) {
            // return $client;
            echo($id);
            $paymentService[$id] = ["client" => $client->name_client ,"idCS" => $id , "expired year" =>$this->getExpiredPaymentCS($id)];
